<template>
  <page-header title="메인 인스타그램 관리" breadcrumbsDepth1="메인 인스타그램 관리" />
  <div class="layout fluid page-container">
            ...
            <tr :key="element.id">
              ...
              <td>
                <va-button
                  ...
                  size="medium"
                  :to="{ name: 'main-instagram-edit', params: { id: element.id } }"
                  >수정</va-button
                >
    ...
    <div class="row" style="justify-content: end">
      <va-button :to="{ name: 'main-instagram-create' }">등록</va-button>
    </div>
  </div>
</template>
...
<script setup lang="ts">
  ...
  onMounted(async () => {
    try {
      const result = await requestGet('main-instagram', {})
  ...
  const onDelete = async (item) => {
    init({
      ...
      onOk: async () => {
        try {
          let result = await requestDelete(`main-instagram/${item.id}`, {})
          console.log(result)
          toast('삭제하였습니다.')
