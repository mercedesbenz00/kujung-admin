<template>
  <page-header title="메인 시공사례 수정" breadcrumbsDepth1="최신 인테리어팁" breadcrumbsDepth2="메인 시공사례 수정" />
  <div class="layout fluid page-container">
    ...
    <va-image v-if="originalThumbnailImageUrl !== ''" :src="originalThumbnailImageUrl" style="width: 100px" />
...
<script setup lang="ts">
  ...
  const originalThumbnailImageUrl = ref<string>('')
  const thumbnail = ref([])
  ...
  onMounted(async () => {
    try {
      var result = (await requestGet(`main-construction-case/${id}`, {})).data
      title.value = result.title
      url.value = result.url
      originalThumbnailImageUrl.value = result.thumb_url
  ...
  const onSave = async () => {
    ...
    try {
      var thumbnailUrl = originalThumbnailImageUrl.value
      ...
      await requestPut(`main-construction-case/${id}`, {
        title: title.value,
        url: url.value,
