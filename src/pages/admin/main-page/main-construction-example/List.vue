<template>
  <page-header title="최신 인테리어팁" breadcrumbsDepth1="최신 인테리어팁" />
  <div class="layout fluid page-container">
            ...
            <tr :key="element.id">
              ...
              <td>
                <va-button
                  ...
                  size="medium"
                  :to="{ name: 'main-construction-example-edit', params: { id: element.id } }"
                  >수정</va-button
                >
    ...
    <div class="row" style="justify-content: end">
      <va-button :to="{ name: 'main-construction-example-create' }">등록</va-button>
    </div>
  </div>
</template>
...
<script setup lang="ts">
  ...
  onMounted(async () => {
    try {
      const result = await requestGet('main-construction-case', {})
  ...
  const onDelete = async (item) => {
    init({
      ...
      onOk: async () => {
        try {
          let result = await requestDelete(`main-construction-case/${item.id}`, {})
          console.log(result)
          toast('삭제하였습니다.')
