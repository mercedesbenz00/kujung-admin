<template>
  <page-header title="베스트 제품 메인 노출 관리" breadcrumbsDepth1="베스트 제품 메인 노출 관리" />
        ...
        <draggable v-model="products" tag="tbody" item-key="id" handle=".handle" @change="onOrderChanged">
          <template #item="{ element }">
            <tr :key="element.id">
              ...
              <td>
                <va-button @click="() => onDelete(element)">베스트상품 메인 노출 해제</va-button>
              </td>
            </tr>
          </template>
        </draggable>
...
<script setup lang="ts">
  ...
  const onDelete = async (product) => {
    init({
      ...
      onOk: async () => {
        try {
          let productSelectedIcons = product.selected_icons.split(',').filter((it) => it != 'Best')

          let result = await requestPut(`product/${product.id}`, {
            selected_icons: productSelectedIcons.join(','),
          })
          console.log(result)
          toast('해제하였습니다.')
  ...
  const onProductSelected = async (product) => {
    try {
      let productSelectedIcons = product.selected_icons.split(',')
      productSelectedIcons = ['Best', ...productSelectedIcons]
