<template>
  <page-header title="제품 관리법" breadcrumbsDepth1="제품 관리법" />
  <div class="layout fluid page-container">
    ...
    <div class="row" style="justify-content: end">
      <va-button :to="{ name: 'product-manage-method-create' }">등록</va-button>
    </div>
  </div>
</template>
...
<script setup lang="ts">
  ...
  const onSearch = async () => {
    try {
      const result = await requestGet('homepage/management-law', {
  ...
  const onRowClick = (id) => {
    router.replace({ name: 'product-manage-method-edit', params: { id: id } })
  }
  ...
  const onDelete = (element) => {
    alert({
      ...
      onOk: async () => {
        try {
          let result = await requestDelete(`homepage/management-law/${element.id}`, {})
          console.log(result)
          toast('삭제하였습니다.')
