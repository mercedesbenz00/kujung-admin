<template>
  <page-header title="인테리어 트렌드" breadcrumbsDepth1="인테리어 트렌드" />
  <div class="layout fluid page-container">
    ...
    <div class="row" style="justify-content: end">
      <va-button :to="{ name: 'interior-trend-create' }">등록</va-button>
    </div>
  </div>
</template>
...
const onSearch = async () => {
  try {
    const result = await requestGet('homepage/interior-trend', {
...
const onRowClick = (id) => {
  router.replace({ name: 'interior-trend-edit', params: { id: id } })
}
...
const onDelete = (element) => {
  alert({
    ...
    onOk: async () => {
      try {
        let result = await requestDelete(`homepage/interior-trend/${element.id}`, {})
        console.log(result)
        toast('삭제하였습니다.')
