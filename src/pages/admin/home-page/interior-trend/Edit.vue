<template>
  <page-header title="인테리어 트렌드 수정" breadcrumbsDepth1="인테리어 트렌드" breadcrumbsDepth2="인테리어 트렌드 수정" />
  <div class="layout fluid page-container">
    ...
    <iframe :src="`https://www.youtube.com/embed/${Common.getYoutubeIdFromURL(youtubeUrl)}?autoplay=0&rel=0&loop=1`"
      style="width: 200px; height: 200px" frameborder="0"
      allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" allowfullscreen />
    <va-divider />

    <div class="row" style="justify-content: end">
      <va-button preset="secondary" border-color="primary" size="medium"
        :to="{ name: 'interior-trend-list' }">취소</va-button>
...
onMounted(async () => {
  try {
    var result = (await requestGet(`homepage/interior-trend/${id.value}`, {})).data
...
const onSave = async () => {
  ...
  try {
    await requestPut(`homepage/interior-trend/${id.value}`, {
      title: title.value,
    ...
    router.replace({ name: 'interior-trend-list' })
  } catch (error) {
    console.log(error)
  }
}
