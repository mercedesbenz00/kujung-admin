<template>
  <page-header title="공지지항" breadcrumbsDepth1="공지지항" />
  <div class="layout fluid page-container">
    ...
    <div class="row" style="justify-content: end">
      <va-button :to="{ name: 'notice-create' }">등록</va-button>
    </div>
  </div>
</template>
...
<script setup lang="ts">
  ...
  const onSearch = async () => {
    try {
      const result = await requestGet('homepage/notification', {
  ...
  const onRowClick = (id) => {
    router.replace({ name: 'notice-detail', params: { id: id } })
  }
</script>
