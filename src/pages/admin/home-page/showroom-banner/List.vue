<template>
  <page-header title="본사 쇼룸 배너 관리" breadcrumbsDepth1="본사 쇼룸 배너 관리" />
  <div class="layout fluid page-container">
    <div class="row" style="justify-content: end">
      <va-button :to="{ name: 'showroom-banner-create' }">등록</va-button>
    </div>
    <va-divider />
    <div class="row">
      <div
      ...
      >
        <div style="display: flex; flex-direction: column; align-items: center; justify-content: center; height: 150px">
          <va-image :src="item.image_url" style="width: 100px; height: 100px" />
        </div>
        <div
        ...
        >
          <div style="margin-top: 20px">
            <va-button preset="secondary" border-color="primary" size="medium" @click="() => onDelete(item)"
              >삭제</va-button
            >
            <va-button style="margin-left: 10px" :to="{ name: 'showroom-banner-edit', params: { id: item.id } }"
              >수정</va-button
            >
          </div>
        </div>
...
<script setup lang="ts">
  ...
  onMounted(async () => {
    try {
      var result = await requestGet('homepage/showroom-banner', {})
  ...
  const onDelete = async (element) => {
    init({
      ...
      onOk: async () => {
        try {
          let result = await requestDelete(`homepage/showroom-banner/${element.id}`, {})
          console.log(result)
          toast('삭제하였습니다.')
