...
<script setup lang="ts">
  ...
  const type = ref<ICommonConstant[]>([])

  const statusValues = ['답변대기', '처리완료', '보류']
  const status = ref<string[]>([])

  const keywordTypes = ref<string[]>(['신청자명', '전화번호'])
  const keywordTypeValues = { 신청자명: 'requester.name', 전화번호: 'phone' }
  ...
  const onSearch = async () => {
    try {
      const result = await requestPost('question-and-answer/list', {
        from: from.value,
        to: to.value,
        q: keyword.value,
        q_type: keywordTypeValues[keywordType.value],
        question_type_list: getConstantsIds(type.value),
