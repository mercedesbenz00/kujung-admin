<template>
  <page-header v-if="isLanHousewarming" title="랜선 집들이" breadcrumbsDepth1="랜선 집들이" />
  <page-header v-if="!isLanHousewarming" title="전문가 집들이" breadcrumbsDepth1="전문가 집들이" />
      ...
      <div class="row">
        ...
        <div class="flex flex-col search-form-value">
          <div class="row" style="align-items: center">
            <va-select v-model="dateType" :options="dateTypes" style="width: 200px; margin-right: 10px" />
      ...
      <table class="va-table">
        ...
        <tbody>
          <tr v-for="element in data" :key="element.id" @click="onRowClick(element.id)">
            ...
            <td>
              <va-button v-if="element.main_display" @click.stop="onRemoveMainExposure(element)"
                >메인노출해제</va-button
              >
              <va-button v-if="element.main_display === false" @click.stop="onAddMainExposure(element)"
                >메인노출등록</va-button
              >
            </td>
          </tr>
...
<script setup lang="ts">
  ...
  const dateTypes = ref<string[]>(['발행요청일시', '발행승인(반려)일시'])
  const dateTypeValues = { 발행요청일시: 'requested_at', '발행승인(반려)일시': 'approved_at' }
  ...
  const onAddMainExposure = async (element) => {
    try {
  ...
  const onRemoveMainExposure = async (element) => {
    try {
      await requestPut(`${apiUrlPrefix}/${element.id}`, {
        main_display: false,
      })
