...
<script setup lang="ts">
  ...
  const props = defineProps<{
    constant: ICommonConstant | undefined
  }>()
  ...
  onMounted(async () => {
    name.value = props.constant!.name
    priority.value = `${props.constant!.priority}`
    display.value = props.constant!.display ? '노출' : '미노출'
  })
  ...
  const onSave = async () => {
    ...
    try {
      await requestPut(`common-constant/${props.constant!.id}`, {
        name: name.value,
