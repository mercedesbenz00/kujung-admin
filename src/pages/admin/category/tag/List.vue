<template>
  <page-header title="태그" breadcrumbsDepth1="태그" />
  <div class="layout fluid page-container">
    ...
    <va-divider />
    <div class="row">
      <va-input v-model="name" placeholder="태그명을 입력해주세요." />
      <va-input
        v-model="priority"
  ...
  <tag-edit-popup v-if="showTagEditPopup" @saved="onEdited" @closed="showTagEditPopup = false" :tag="tagToEdit" />
</template>
...
<script setup lang="ts">
  ...
  const showTagEditPopup = ref<boolean>(false)
  ...
  const onMainDisplayOff = async (element: ITag) => {
    try {
      await requestPut(`tag/${element.id}`, {
        main_display: false,
      })
      toast('변경하였습니다.')
  ...
  const onMainDisplayOn = async (element: ITag) => {
    try {
      await requestPut(`tag/${element.id}`, {
        main_display: true,
      })
  ...
  const onEdit = (element: ITag) => {
    tagToEdit.value = element
    showTagEditPopup.value = true
  }
  ...
  const onDelete = (element: ITag) => {
    alert({
      ...
      onOk: async () => {
        try {
          let result = await requestDelete(`tag/${element.id}`, {})
          console.log(result)
          toast('삭제하였습니다.')
