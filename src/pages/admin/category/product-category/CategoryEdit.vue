<template>
  <div class="layout fluid" style="padding: 20px; border-style: solid; border-width: 1px; border-color: #e5e5e5">
    <div class="label">고유코드*</div>
    <va-input v-model="category.id" disabled />

    <div class="label">{{ Depths[category.depth] }}명*</div>
    ...
    <va-image :src="Common.getThumbnailUrl(originalMainImageUrl)" style="width: 100px; height: auto" />
    <va-file-upload v-model="mainImage" />
    <div v-if="mainImage.length === 0">선택된 파일이 없습니다.</div>
    <va-divider />

    <div class="label">소개 이미지*</div>
    ...
    <div class="row" style="margin-top: 20px; align-items: center; margin-left: 0px; margin-right: 0px">
      추가:
      <va-input
        v-model="youtubeUrl"
    ...
    <div class="row" style="display: flex; flex-direction: row; justify-content: end">
      ...
      <va-button size="medium" style="margin-left: 10px" @click="onSave">수정완료</va-button>
    </div>
  </div>
...
<script setup lang="ts">
  ...
  import { requestPut, uploadFile, uploadFiles } from '../../../../util/api'
  ...
  const onSave = async () => {
    ...
    try {
      ...
      await requestPut(`category/${props.category.id}`, {
        name: name.value,
