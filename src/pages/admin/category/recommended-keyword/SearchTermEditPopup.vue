      ...
      <div class="layout fluid" style="width: 1000px">
        <va-navbar color="#6B737F">
          <template #left>
            <va-navbar-item class="navbar-item-slot">
              <span class="title">추천검색어 수정</span>
            </va-navbar-item>
          </template>
          <template #right>
            <va-navbar-item class="navbar-item-slot">
        ...
        <div class="layout" style="padding: 30px">
          <div class="row" style="align-items: center">
            추천검색어:<va-input v-model="name" placeholder="추천검색어를 입력해주세요." style="margin-start: 10px; margin-end: 20px" />
            우선순위:<va-input v-model="priority" placeholder="우선순위를 입력해주세요." style="margin-start: 10px; margin-end: 20px" />
            노출여부:
...
const props = defineProps<{
  searchTerm: ISearchTerm | undefined
}>()
...
onMounted(async () => {
  name.value = props.searchTerm!.name
  priority.value = `${props.searchTerm!.priority}`
  display.value = props.searchTerm!.display ? '노출' : '미노출'
})
...
const onSave = async () => {
  ...
  try {
    await requestPut(`search-term/${props.searchTerm!.id}`, {
      name: name.value,
